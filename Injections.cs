...
namespace RootsBeta
{
    public partial class RootsBeta
    {
        ...
        {
            if (!self.TryGetGlobalNPC<AIOverrideSystem>(out var system) || system.CurrentAiOverride is not Snatcher || hit.DamageType == ModContent.GetInstance<PickaxeDamage>())
            ...
            CombatText.NewText(new Rectangle((int)self.position.X, (int)self.position.Y, self.width, self.height), color2, 0);
        ...
        {
            if (self.GetGlobalNPC<AIOverrideSystem>().CurrentAiOverride is not Snatcher || noPlayerInteraction)
        ...
        private void SnatcherSmartCursor(On_SmartCursorHelper.orig_SmartCursorLookup orig, Player player)
        {
            ...
            var reachableEndX = (int)((player.position.X + player.width) / 16f) + Player.tileRangeX + player.HeldItem.tileBoost - 1;
            var reachableStartY = (int)(player.position.Y / 16f) - Player.tileRangeY - player.HeldItem.tileBoost + 1;
            var reachableEndY = (int)((player.position.Y + player.height) / 16f) + Player.tileRangeY + player.HeldItem.tileBoost - 2;
            ...
            for (int i = reachableStartX; i <= reachableEndX; i++)
            {
                ...
                {
                    if (!snatchers.Any(x => (int)x.ai[0] == i && (int)x.ai[1] == j)) continue;
        ...
        private void SpectreHurt(On_Projectile.orig_ghostHurt orig, Projectile self, int dmg, Vector2 position, Entity victim)
        {
            ...
            const int num3 = 1500;
            if (Main.LocalPlayer.ghostDmg > num3)
            {
                return;
            }
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                float num6 = Math.Abs(Main.npc[i].position.X + (Main.npc[i].width / 2) - self.position.X + (self.width / 2)) + Math.Abs(Main.npc[i].position.Y + (Main.npc[i].height / 2) - self.position.Y + (self.height / 2));
                if (!(num6 < 800f)) continue;
            ...
            if (num4 == 0 && num5 == 0) return;
            var num2 = ((num5 <= 0) ? array[Main.rand.Next(num4)] : array[Main.rand.Next(num5)]);
        ...
        private void SpectreHeal(On_Projectile.orig_ghostHeal orig, Projectile self, int dmg, Vector2 position, Entity victim)
        {
            ...
            float num2 = dmg * num;
            if ((int)num2 <= 0 || Main.LocalPlayer.lifeSteal <= 0f)
            {
                return;
            }

            Main.LocalPlayer.lifeSteal -= num2;
            float num3 = 0f;
            int num4 = self.owner;
            for (int i = 0; i < 255; i++)
            {
                if (!Main.player[i].active || Main.player[i].dead ||
                    ((Main.player[self.owner].hostile || Main.player[i].hostile) &&
                     Main.player[self.owner].team != Main.player[i].team) ||
                    !(self.Distance(Main.player[i].Center) <= 3000f)) continue;
                int num5 = Main.player[i].statLifeMax2 - Main.player[i].statLife;
                if (!(num5 > num3)) continue;
                num3 = num5;
                num4 = i;
            }
